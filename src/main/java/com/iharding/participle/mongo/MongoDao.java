...
//      }
        ...
        while (cursor.hasNext()) {
            ...
            //把json字符串转换成map格式,为后面做准备,方法放在后面
...
//    @Override
    public static boolean insert(MongoDatabase db, String table, Document document) {
        MongoCollection<Document> collection = db.getCollection(table);
...
//      System.out.println(collection.getWriteConcern().getW());//1
     ...
     * @param documents
     */
    public static boolean insertMany(MongoDatabase db, String table, List<Document> documents ) {
...
//    @Override
    public static boolean update(MongoDatabase db, String table, BasicDBObject whereDoc,BasicDBObject updateDoc) {
        MongoCollection<Document> collection = db.getCollection(table);
     ...
     * @param updateDoc
     */
    public static boolean updateOne(MongoDatabase db, String table, BasicDBObject whereDoc, BasicDBObject updateDoc, UpdateOptions option) {
        MongoCollection<Document> collection = db.getCollection(table);
//        System.out.println(whereDoc);
//        System.out.println(updateDoc);
//        System.out.println(option.isUpsert());
        UpdateResult updateOneResult = collection.updateOne(whereDoc, new Document("$set",updateDoc), option);
//        long modifiedCount = updateOneResult.getModifiedCount();
//        System.out.println("修改的数量: "+modifiedCount);
//        if (updateOneResult.getUpsertedId() != null)
//            System.out.println("Upserted ID: " + updateOneResult.getUpsertedId());
        /*
        if(modifiedCount == 1){
